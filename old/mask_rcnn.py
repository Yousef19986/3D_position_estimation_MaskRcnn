...
def get_xyz(intr,depth_frame,center):
	...
	return int(Xtarget), int(Ytarget), int(Ztarget)
...
class MaskRCNN:
    ...
    def draw_object_info(self, bgr_frame, depth_frame,intr,depth):
        # loop through the detection
        for box, class_id, obj_center in zip(self.obj_boxes, self.obj_classes, self.obj_centers):
            ...
            cv2.putText(bgr_frame, f"{get_xyz(intr,depth,(cx,cy))}", (x + 5, y + 60), 0, 0.5, (255, 255, 255), 2)
            cv2.rectangle(bgr_frame, (x, y), (x2, y2), color, 1)
            cv2.circle(bgr_frame,(cx,cy),1,(255,255,255),2)
